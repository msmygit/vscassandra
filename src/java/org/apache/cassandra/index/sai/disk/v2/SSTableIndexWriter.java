...
public class SSTableIndexWriter implements PerIndexWriter
{
    ...
    private final List<SegmentMetadata> unmergedSegments = new ArrayList<>();
    private SegmentMetadata mergedSegment;
    ...
    public void complete(Stopwatch stopwatch) throws IOException
    {
        ...
        finally
        {
            ...
            {
                indexContext.getIndexMetrics().segmentsPerCompaction.update(unmergedSegments.size());
    ...
    private void flushSegment() throws IOException
    {
        ...
        try
        {
            ...
            {
                unmergedSegments.add(segmentMetadata);
    ...
    {
        if (unmergedSegments.isEmpty())
            return;

        PrimaryKey minKey = unmergedSegments.get(0).minKey;
        PrimaryKey maxKey = unmergedSegments.get(unmergedSegments.size() - 1).maxKey;
        ...
        {
            for (final SegmentMetadata segment : unmergedSegments)
            {
                segmentMerger.addSegment(indexContext, segment, perIndexFiles);
            }
            mergedSegment = segmentMerger.merge(indexDescriptor, indexContext, minKey, maxKey, maxSSTableRowId);
        }
        finally
        {
    ...
    private void writeSegmentsMetadata() throws IOException
    {
        ...
        {
            SegmentMetadata.write(writer, mergedSegment);
        }
        catch (IOException e)
        {
            abort(e);
