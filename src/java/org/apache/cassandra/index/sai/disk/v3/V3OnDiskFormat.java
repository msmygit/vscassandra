...
/**
 * Version 3 block terms and inverted index combination index disk format.
 *
 * See {@link BlockTerms} for more details.
 */
@NotThreadSafe
public class V3OnDiskFormat extends V2OnDiskFormat
{
    ...
    private void validateCRC(IndexInput input, FileValidation.Meta meta) throws IOException
    {
        if (!FileValidation.validate(input, meta))
            throw new IOException("Validation failed file="+input.toString());
    }
                                              ...
                                              boolean checksum)
    {
        ...
        for (final IndexComponent indexComponent : V3_INDEX_COMPONENTS)
        {
            if (!isBuildCompletionMarker(indexComponent))
            {
                ...
                {
                    logger.error(indexDescriptor.logMessage("{} failed for index component {} on SSTable {}"),
    ...
    @Override
    // number of files excluding COLUMN_COMPLETION_MARKER and META
