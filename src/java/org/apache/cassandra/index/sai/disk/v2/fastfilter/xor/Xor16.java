package org.apache.cassandra.index.sai.disk.v2.fastfilter.xor;

import java.io.IOException;

import org.apache.cassandra.index.sai.disk.v2.fastfilter.Filter;
import org.apache.cassandra.index.sai.disk.v2.fastfilter.utils.Hash;
...
public class Xor16 implements Filter
{
    ...
    public Xor16(IndexInput input) throws IOException
    {
        bitCount = input.readInt();
        blockLength = input.readInt();
        seed = input.readLong();
        int len = input.readInt();
        fingerprints = new short[len];
        for (int x = 0; x < fingerprints.length; x++)
        {
            fingerprints[x] = input.readShort();
        }
    }

    public void write(IndexOutput output) throws IOException
    {
        output.writeInt(bitCount);
        output.writeInt(blockLength);
        output.writeLong(seed);
        output.writeInt(fingerprints.length);
        for (int x = 0; x < fingerprints.length; x++)
        {
            output.writeShort(fingerprints[x]);
        }
    }
