...
public class V3SSTableIndexWriter extends SSTableIndexWriter
{
    ...
    public static class V3RAMStringSegmentBuilder extends SegmentBuilder.RAMStringSegmentBuilder
    {
        ...
        {
            if (indexContext.isSegmentCompactionEnabled())
            {
                // only write the block terms index for segments, the equality index is not required for segment compaction
                final BlockTermsWriter writer = new BlockTermsWriter(indexDescriptor, indexContext, true);
                return writer.writeAll(ramIndexer.getTermsWithPostings(), null);
            }
            else
            {
                try (final V3InvertedIndexWriter invertedWriter = new V3InvertedIndexWriter(indexDescriptor, indexContext, false))
                {
                    final BlockTermsWriter writer = new BlockTermsWriter(indexDescriptor, indexContext, false);
                    final SegmentMetadata.ComponentMetadataMap indexMetas = writer.writeAll(ramIndexer.getTermsWithPostings(), invertedWriter);
                    return indexMetas;
                }
            }
        }
