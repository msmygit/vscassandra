...
public final class MergingIterator implements Iterator<ByteComparable>
{
    ...
    private void pullTop()
    {
        ...
        while (queue.size() != 0
               && ByteComparable.compare(queue.top().current, top[0].current, ByteComparable.Version.OSS42) == 0)
        {
            top[numTop++] = queue.pop();
        }
    ...
    private static class TermMergeQueue extends PriorityQueue<SubIterator>
    {
        ...
        {
            final int cmp = ByteComparable.compare(a.current, b.current, ByteComparable.Version.OSS42);
