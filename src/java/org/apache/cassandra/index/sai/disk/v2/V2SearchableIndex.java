...
public class V2SearchableIndex implements SearchableIndex
{
    ...
    public V2SearchableIndex(SSTableContext sstableContext, IndexContext indexContext)
    {
        ...
        try
        {
            ...
            metadata = SegmentMetadata.load(source, sstableContext.indexDescriptor.primaryKeyFactory);

            segment = new Segment(indexContext, sstableContext, indexFiles, metadata);

            this.minKey = metadata.minKey.partitionKey();
            this.maxKey = metadata.maxKey.partitionKey();

            this.minTerm = metadata.minTerm;
            this.maxTerm = metadata.maxTerm;
            this.sketch = metadata.sketch;
            this.numRows = metadata.numRows;

            this.minSSTableRowId = metadata.minSSTableRowId;
            this.maxSSTableRowId = metadata.maxSSTableRowId;
        }
        catch (Throwable t)
        {
