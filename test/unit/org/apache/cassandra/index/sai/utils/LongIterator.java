...
{
    private static PrimaryKey.PrimaryKeyFactory primaryKeyFactory = PrimaryKey.factory(Murmur3Partitioner.instance,
    ...
    {
        return primaryKeyFactory.createKey(new BufferDecoratedKey(new Murmur3Partitioner.LongToken(token), ByteBufferUtil.bytes(token)));
    }

    public static PrimaryKey fromTokenAndRowId(long token, long rowId)
    {
        return primaryKeyFactory.createKey(new BufferDecoratedKey(new Murmur3Partitioner.LongToken(token),
                                                                  ByteBufferUtil.bytes(token)),
                                           Clustering.EMPTY,
                                           rowId);
    }
