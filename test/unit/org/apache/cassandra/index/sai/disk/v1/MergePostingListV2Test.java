...
public class MergePostingListV2Test extends NdiRandomizedTest
{
    @Test
    public void shouldMergeInterleavedPostingLists() throws IOException
    {
        final List<PostingList.PeekablePostingList> lists = Lists.newArrayList(
                new ArrayPostingList(new int[]{ 1, 4, 6 }).peekable(),
                new ArrayPostingList(new int[]{ 2, 3, 4 }).peekable(),
                new ArrayPostingList(new int[]{ 1, 6 }).peekable(),
                new ArrayPostingList(new int[]{ 2, 5 }).peekable(),
                new ArrayPostingList(new int[]{ 3, 6 }).peekable(),
                new ArrayPostingList(new int[]{ 3, 5, 6 }).peekable());

        final PostingList merged = MergePostingListV2.merge(lists);
    ...
    public void shouldMergeDisjointPostingLists() throws IOException
    {
        final List<PostingList.PeekablePostingList> lists = Lists.newArrayList(
                new ArrayPostingList(new int[]{ 1, 6 }).peekable(),
                new ArrayPostingList(new int[]{ 8, 9, 11 }).peekable(),
                new ArrayPostingList(new int[]{ 15 }).peekable());
    ...
    {
        final List<PostingList.PeekablePostingList> lists = Lists.newArrayList(new ArrayPostingList(new int[]{ 1, 4, 6 }).peekable());
...
//
//        assertEquals(expected.advance(V2PrimaryKeyMap.IDENTITY.primaryKeyFromRowId(9)), merged.advance(V2PrimaryKeyMap.IDENTITY.primaryKeyFromRowId(9)));
//
...
//        final PostingList merged = MergePostingList.merge(lists);
//
//        assertEquals(2, merged.advance(V2PrimaryKeyMap.IDENTITY.primaryKeyFromRowId(2)));
//        assertEquals(4, merged.advance(V2PrimaryKeyMap.IDENTITY.primaryKeyFromRowId(4)));
...
//        assertEquals(5, merged.advance(V2PrimaryKeyMap.IDENTITY.primaryKeyFromRowId(5)));
...
//        final PostingList merged = MergePostingList.merge(lists);
//        assertEquals(1, merged.nextPosting());
//        assertEquals(3, merged.advance(V2PrimaryKeyMap.IDENTITY.primaryKeyFromRowId(3)));
//        assertEquals(4, merged.advance(V2PrimaryKeyMap.IDENTITY.primaryKeyFromRowId(4)));
//    }
//
//    @Test
            ...
            else
            {
                ...
                actions.add((postingList) -> {
                    while (true)
                    {
                        try
                        {
                            return postingList.advance(V2PrimaryKeyMap.IDENTITY.primaryKeyFromRowId(rowID));
                        }
    ...
    private void testAdvancingToAllElements()
    {
        ...
        for (int targetRowID : mergedPostings)
        {
            ...
            while (true)
            {
                try
                {
                    rowID = merged.advance(V2PrimaryKeyMap.IDENTITY.primaryKeyFromRowId(targetRowID));
                    break;
                }
                catch (ArrayPostingList.LookupException ignore)
                {
