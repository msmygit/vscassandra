...
public abstract class MultiLevelSkipListReader implements Closeable {
  ...
  private Lucene8xIndexInput[] skipStream;
  ...
  protected FixedBitSet blockIdBitSet = null;

  /** Creates a {@code MultiLevelSkipListReader}. */
  protected MultiLevelSkipListReader(Lucene8xIndexInput skipStream, int maxSkipLevels, int skipInterval, int skipMultiplier) {
    this.skipStream = new Lucene8xIndexInput[maxSkipLevels];
    this.skipPointer = new long[maxSkipLevels];
    this.childPointer = new long[maxSkipLevels];
    this.numSkipped = new long[maxSkipLevels];
    ...
    skipDoc = new long[maxSkipLevels];
  }

  /** Creates a {@code MultiLevelSkipListReader}, where
   *  {@code skipInterval} and {@code skipMultiplier} are
   *  the same. */
  protected MultiLevelSkipListReader(Lucene8xIndexInput skipStream, int maxSkipLevels, int skipInterval) {
    this(skipStream, maxSkipLevels, skipInterval, skipInterval);
  }
  
  /** Returns the id of the doc to which the last call of {@link #skipTo(long)}
   *  has skipped.  */
  public long getDoc() {
    return lastDoc;
  }
   ...
   */
  public long skipTo(long target) throws IOException {
    ...
    while (level >= 0) {
      long newDocUpto = numSkipped[level];
      int blockIdx = ((int)newDocUpto / 128);

      // System.out.println("level="+level+" blockIdx="+blockIdx+" newDocUpto="+newDocUpto);

      //if (target > skipDoc[level] || blockIdx >= 2) {
      //if (target > skipDoc[level] || (blockIdx <= 8 && blockIdx >= 1) ) {
      boolean noUse = blockIdBitSet != null && !blockIdBitSet.get(blockIdx);
      if (target > skipDoc[level] && noUse) {// || noUse && target < skipDoc[level]) {
          boolean skipLoaded = loadNextSkip(level);
          if (!skipLoaded)
          {
            //level--;
            continue;
...
//        }
    ...
    //System.out.println("numSkipped[0]="+numSkipped[0]+" skipInterval[0]="+skipInterval[0]);
  ...
  private boolean loadNextSkip(int level) throws IOException {
    ...
    //System.out.println("numSkipped level="+level+" skipped="+numSkipped[level]);
   ...
   */  
  protected abstract long readSkipData(int level, Lucene8xIndexInput skipStream) throws IOException;
  ...
  private final static class SkipBuffer extends Lucene8xIndexInput {
    private byte[] data;
    private long pointer;
    private int pos;
    
    SkipBuffer(Lucene8xIndexInput input, int length) throws IOException {
