...
public class V3LegacyIndexCqlTest extends SAITester
{
    @Test
    public void loadLegacyIndexAndRunCqlQuery() throws Throwable
    {
        ...
        assertThatThrownBy(() -> executeFormattedQuery("SELECT * FROM " + KEYSPACE + '.' + currentTable() + " WHERE text_value LIKE '1%'")).isInstanceOf(IllegalArgumentException.class);
        assertThatThrownBy(() -> executeFormattedQuery("SELECT * FROM " + KEYSPACE + '.' + currentTable() + " WHERE text_value > '1' AND text_value < '10'")).isInstanceOf(IllegalArgumentException.class);
        ...
        resultSet = executeFormattedQuery("SELECT * FROM " + KEYSPACE + '.' + currentTable() + " WHERE text_value LIKE '1%'");
        assertEquals(111, resultSet.size());

        resultSet = executeFormattedQuery("SELECT * FROM " + KEYSPACE + '.' + currentTable() + " WHERE text_value > '1' AND text_value < '150'");
        assertEquals(56, resultSet.size());
